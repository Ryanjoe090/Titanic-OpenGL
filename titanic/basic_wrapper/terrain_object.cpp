...
void terrain_object::calculateNoise()
{
	...
	GLfloat* noiseArrys [5]; 
	noise = new GLfloat[xsize * zsize * perlin_octaves];
	for (int arrays = 0; arrays < 5; arrays++)
	{
		noiseArrys[arrays] = new GLfloat[xsize * zsize * perlin_octaves];
		for (int i = 0; i < (xsize*zsize*perlin_octaves); i++)
		{
			noiseArrys[arrays][i] = 0;
		}


		GLfloat xfactor = 1.f / (xsize - 1);
		GLfloat zfactor = 1.f / (zsize - 1);
		GLfloat freq = fmod( perlin_freq * rand(),  7 );
		...
		for (int row = 0; row < zsize; row++)
		{
			for (int col = 0; col < xsize; col++)
			{
				...
				for (int oct = 0; oct < perlin_octaves; oct++)
				{
					...
					noiseArrys[arrays][(row * xsize + col) * perlin_octaves + oct] = result;
	...
	for (int k = 0; k < (xsize * zsize * perlin_octaves); k++)
	{
		noise[k] = ((noiseArrys[0][k]) + noiseArrys[1][k] + noiseArrys[2][k] + noiseArrys[3][k] + noiseArrys[4][k]) / 5;
	}
}
