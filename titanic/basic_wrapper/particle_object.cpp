...
void particle_object::create(GLuint program)
{
	...
	Texture = SOIL_load_OGL_texture("smoke.png", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID,
...
void particle_object::drawParticles(glm::mat4 ProjectionMatrix, glm::mat4 ViewMatrix, float smokePos)
{
	...
	for (int i = 0; i<newparticles; i++){
		...
		ParticlesContainer[particleIndex].life = 3.f; // This particle will live 5 seconds.
		ParticlesContainer[particleIndex].pos = glm::vec3(smokePos, 4.20f, 0.1f);
	...
	for (int i = 0; i<MaxParticles; i++){
		...
		if (p.life > 0.0f){
			...
			if (p.life > 0.0f){
				...
				p.speed += glm::vec3(-5.f, 0.2f, 0.0f) * (float)delta * 0.5f;
				p.pos += p.speed * (float)delta * 0.1f;
